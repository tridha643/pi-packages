...
const CLAUDE_TOOL_NAME_BY_PI_NAME: Readonly<Record<string, string>> = {
  read: "Read",
  bash: "Bash",
  edit: "Edit",
  write: "Write",
...
function resolveClaudeAllowedTools(tools: ReadonlyArray<string> | undefined) {
  if (!tools) return undefined;
  return [...new Set(tools.map((tool) => CLAUDE_TOOL_NAME_BY_PI_NAME[tool] ?? tool))];
}
...
function isTopLevelClaudeActivity(message: SDKMessage) {
  if (
...
const makeClaudeSession = (
  ...
  Effect.gen(function* () {
    ...
    type InitialPreflight =
      | { readonly _tag: "Accepted" }
      | { readonly _tag: "SpawnFailed"; readonly message: string };
    let preflightAccepted = false;
    let preflightSettled = false;
    let preflightTimer: ReturnType<typeof setTimeout> | undefined;
    let resolvePreflight = (_result: InitialPreflight) => {};
    const initialPreflight = new Promise<InitialPreflight>((resolve) => {
      resolvePreflight = resolve;
    });
    const finishPreflight = (result: InitialPreflight) => {
      if (preflightSettled) return;
      preflightSettled = true;
      if (preflightTimer) clearTimeout(preflightTimer);
      resolvePreflight(result);
    };
    const acceptPreflight = () => {
      preflightAccepted = true;
      finishPreflight({ _tag: "Accepted" });
    };
      ...
      try: () =>
        query({
          ...
          options: {
            ...
            ...(task.allowedTools
              ? { allowedTools: resolveClaudeAllowedTools(task.allowedTools) }
              : {}),
      ...
      if (!preflightAccepted) {
        finishPreflight({
          ...
          message: failureDetails
            ? boundedError(failureDetails)
            : "Claude Code completed before emitting top-level assistant, stream, or tool activity.",
        });
      }
      ...
      if (state.interruptRequested) {
        ...
        settle({
          ...
          errorText: boundedError(
            failureDetails ?? `Claude Code ended with ${result.subtype}`,
          ),
    ...
    const handleMessage = (message: SDKMessage) => {
      ...
      if (isTopLevelClaudeActivity(message)) acceptPreflight();
      ...
      try {
        ...
        if (!preflightAccepted) {
          finishPreflight({
            ...
            message:
              failure ??
              "Claude Code query ended before emitting top-level assistant, stream, or tool activity.",
          });
        }
        if (!state.closed) {
          if (state.activeRun) {
    ...
    if (!preflightSettled) {
      preflightTimer = setTimeout(
        () =>
          finishPreflight({
            _tag: "SpawnFailed",
            message: `Claude Code preflight timed out after ${PREFLIGHT_TIMEOUT_MS}ms.`,
          }),
        PREFLIGHT_TIMEOUT_MS,
      );
    }
    const preflight = yield* Effect.promise(() => initialPreflight);
    if (preflight._tag === "SpawnFailed") {
      return yield* new SpawnError({ message: preflight.message });
    }
...
export const claudeBackend: SubagentBackend = {
  ...
  // The SDK bundles a platform executable; a PATH binary is only an optional
  // preference that keeps the child aligned with the user's terminal install.
