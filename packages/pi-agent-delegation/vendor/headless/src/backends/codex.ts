...
class CodexProtocolRejectedError extends Error {}
...
function codexPreflightError(error: unknown) {
  const message = boundedError(error);
  return error instanceof CodexProtocolRejectedError
...
> =>
  Effect.gen(function* () {
    ...
    function startRun(text: string): Promise<Error | undefined> {
      if (state.closed || state.activeRun) return Promise.resolve(undefined);
      const threadId = state.meta.nativeSessionId;
      if (!threadId) {
        return Promise.resolve(
          new Error("Codex session has no thread id for turn/start."),
        );
      }
      ...
      return request("turn/start", params).then(
        (result) => {
          ...
          if (!turnId && !state.activeTurnId) {
            const error = new Error(
              "Codex turn/start returned no turn id; task acceptance is ambiguous.",
            );
            settleRun({ _tag: "Failed", errorText: error.message }, serial);
        ...
        (error: Error) => {
          if (!state.activeRun || serial !== state.runSerial) return error;
    ...
    const handleLine = (line: string) => {
      ...
      if (id !== undefined && pendingRequests.has(id)) {
        ...
        if (message.error !== undefined) {
          pending.reject(
            new CodexProtocolRejectedError(protocolError(message.error)),
          );
        } else {
    ...
    const loadModelList = async () => {
      ...
      while (true) {
        ...
        data.push(...records(page.data));
        const nextCursor = stringValue(page.nextCursor);
        if (!nextCursor || seenCursors.has(nextCursor)) break;
        seenCursors.add(nextCursor);
        cursor = nextCursor;
      }
      return { data } satisfies JsonRecord;
    };
    ...
    if (task.model) {
      ...
      const requestedModelExists = records(modelList.data).some(
        (candidate) =>
          stringValue(candidate.id) === task.model ||
          stringValue(candidate.model) === task.model,
      );
      if (!requestedModelExists) {
        return yield* new BackendPreflightRejectedError({
          message: `Unknown Codex model "${task.model}".`,
        });
      }
    } else if (task.reasoningEffort) {
      // This capability probe is optional when no model needs validation; the
      // unclamped preferred effort remains a safe compatibility fallback.
      modelList = yield* Effect.tryPromise(loadModelList).pipe(
    ...
    const threadResult = yield* Effect.tryPromise({
      ...
      // chose to launch an autonomous subagent, so give the thread full
      // workspace access without interactive approval requests.
      try: () =>
    ...
    emit({ _tag: "MetaChanged", meta: state.meta });
    const initialTurnError = yield* Effect.promise(() => startRun(task.prompt));
    if (initialTurnError) {
      return yield* codexPreflightError(initialTurnError);
    }
