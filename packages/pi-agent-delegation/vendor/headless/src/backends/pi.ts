...
import { openAiFastExtension } from "./openai-fast.ts";
...
/**
 * Load skills, prompts, and trust-gated project context without configured
 * extensions. Loading every global extension per in-process child duplicates
 * MCP servers, terminal listeners, and other long-lived resources in the
 * parent process; the inline OpenAI service-tier hook is intentionally narrow.
 */
  ...
  Effect.gen(function* () {
    ...
    const model = yield* Effect.try({
      try: () =>
        ...
        new BackendPreflightRejectedError({ message: boundedError(error) }),
    });
    ...
    const startRun = (
      ...
      onPromptRejected?: (error: unknown) => void,
    ) => {
      state.runError = undefined;
      state.settled = false;
      emit({ _tag: "RunStarted" });
      void session
        .prompt(text, preflightResult ? { preflightResult } : undefined)
        .catch((error) => {
          state.runError = boundedError(error);
          onPromptRejected?.(error);
    ...
    type InitialPreflight =
      ...
      | { readonly _tag: "PromptFailed"; readonly message: string };
    const initialPreflight = yield* Effect.promise(
      () =>
        new Promise<InitialPreflight>((resolve) => {
          let settled = false;
          let explicitlyRejected = false;
          const finish = (result: InitialPreflight) => {
            if (settled) return;
            settled = true;
            resolve(result);
          };
          startRun(
            task.prompt,
            (accepted) => {
              if (accepted) finish({ _tag: "Accepted" });
              else explicitlyRejected = true;
            },
            (error) =>
              finish({
                _tag: explicitlyRejected ? "Rejected" : "PromptFailed",
                message: boundedError(error),
              }),
          );
        }),
    );
    if (initialPreflight._tag === "Rejected") {
      return yield* new BackendPreflightRejectedError({
        message: initialPreflight.message,
      });
    }
    if (initialPreflight._tag === "PromptFailed") {
      return yield* new SpawnError({ message: initialPreflight.message });
    }
