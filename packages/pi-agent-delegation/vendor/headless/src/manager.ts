...
interface MutableSnapshot {
  ...
  subagentName?: string;
  profileName?: string;
  resultDelivery: "automatic" | "managed";
...
interface Entry {
  ...
  /** Managed chain steps report through their chain rather than individually. */
  resultDelivery: "automatic" | "managed";
...
/** One spawn blocked on manager capacity; queued items own no reservation. */
interface SpawnAdmissionQueueEntry {
  state: SpawnAdmissionState;
  readonly resume: (
    effect: Effect.Effect<void, SpawnError | ConcurrencyLimitError>,
  ) => void;
}
...
export interface SubagentReadModel {
   ...
   * delegate_wait/cancel is collecting the result (so it must not also be
...
const makeManager = Effect.gen(function* () {
  ...
  const spawnAdmissionQueue: SpawnAdmissionQueueEntry[] = [];
  ...
  /** Admit queued spawns in FIFO order without exceeding the global cap. */
  const admitQueuedSpawns = () => {
    if (disposed) return;
    while (
      spawnAdmissionQueue.length > 0 &&
      runningCount() + reserved < MAX_RUNNING
    ) {
      const admission = spawnAdmissionQueue.shift();
      if (!admission || admission.state !== "queued") continue;
      admission.state = "admitted";
      reserved++;
      admission.resume(Effect.void);
    }
  };

  /**
   * Reserve one running slot, waiting in FIFO order when all slots are in use.
   * Interrupting a queued effect removes it before it can own a reservation.
   */
  const acquireSpawnAdmission = Effect.callback<
    void,
  ...
  >((resume) => {
    ...
    const admission: SpawnAdmissionQueueEntry = {
      state: "queued",
      resume,
    };
    spawnAdmissionQueue.push(admission);
    notify();
    return Effect.sync(() => {
      if (admission.state !== "queued") return;
      admission.state = "cancelled";
      const index = spawnAdmissionQueue.indexOf(admission);
      if (index >= 0) spawnAdmissionQueue.splice(index, 1);
      admitQueuedSpawns();
      notify();
    });
  });
  ...
  const settle = (entry: Entry, outcome: RunOutcome) => {
    ...
    s.queued = [];
    const consumed =
      entry.resultDelivery === "managed" || (waitInterest.get(s.id) ?? 0) > 0;
    notify(s.id);
    try {
  ...
  const foldEvent = (entry: Entry, event: SubagentEvent) => {
    ...
    switch (event._tag) {
      case "RunStarted":
        entry.restarting = false;
        if (s.status !== "running") s.runGeneration++;
        s.status = "running";
  ...
  const spawn = (backendName: BackendName, task: SpawnTask) =>
    Effect.uninterruptibleMask((restore) =>
      Effect.gen(function* () {
        // Admission remains interruptible while queued. The surrounding mask
        // closes the interruption gap before the reservation finalizer is
        // attached, so an admitted abort cannot leak its slot.
        yield* restore(acquireSpawnAdmission);
        ...
        const doSpawn = Effect.gen(function* () {
          ...
          const entry: Entry = {
            snapshot: {
              ...
              prompt: task.prompt,
              cwd: task.cwd,
              ...(task.subagentName !== undefined
                ? { subagentName: task.subagentName }
                : {}),
              ...(task.profileName !== undefined
        ...
        return yield* restore(doSpawn).pipe(
          Effect.ensuring(
            Effect.sync(() => {
              reserved--;
              admitQueuedSpawns();
  ...
  const send = (id: string, text: string) =>
    Effect.suspend((): Effect.Effect<void, SendError> => {
      ...
      if (entry.snapshot.status !== "running") {
        ...
        // Make the accepted restart visible synchronously so immediate wait,
        // cancel, and duplicate-continuation checks observe the new run.
        const previous = {
        ...
        entry.snapshot.finalText = "";
        entry.snapshot.runGeneration++;
        notify(id);
        return entry.session.send(text).pipe(
          Effect.onError(() =>
            Effect.sync(() => {
              ...
              entry.snapshot.finalText = previous.finalText;
              entry.snapshot.runGeneration = previous.runGeneration;
              notify(id);
            }),
          ),
        );
  ...
  const disposeAll = Effect.gen(function* () {
    disposed = true;
    for (const admission of spawnAdmissionQueue.splice(0)) {
      if (admission.state !== "queued") continue;
      admission.state = "cancelled";
      admission.resume(
        new SpawnError({
          message: "Subagent manager shut down while waiting to spawn.",
        }),
      );
    }
