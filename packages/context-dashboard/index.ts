...
import {
  createEmptyGitInfoState,
  createEmptyModelInfoState,
  GIT_INFO_CHANNEL,
  isGitInfoState,
  isModelInfoState,
  MODEL_INFO_CHANNEL,
...
type ThemeForeground = ContextWindowBarTone | "text" | "dim";
...
interface DashboardTheme {
  fg(color: ThemeForeground, text: string): string;
}
...
function renderContextWindowBar(
  ...
  const layout = createContextWindowBarLayout(modelInfo, width);
  if (!layout.label) {
    return theme.fg(layout.tone, layout.percentLabel);
  }

  return truncateToWidth(
    [
      theme.fg("muted", layout.label),
      " ",
      theme.fg(layout.tone, layout.filledTrack),
      theme.fg("dim", layout.emptyTrack),
      " ",
      theme.fg(layout.tone, layout.percentLabel),
      theme.fg("dim", layout.details),
    ].join(""),
    width,
    "",
  );
...
/** Installs the local Pi header and responsive context-window dashboard footer. */
export default function contextDashboard(pi: ExtensionAPI) {
  let title = "pi";
  let modelInfo = createEmptyModelInfoState();
  let gitInfo = createEmptyGitInfoState();
      ...
      return {
        ...
        render(width: number) {
          ...
          const model = modelInfo.provider
            ...
            : modelInfo.modelId;
          const styledGit = theme.fg("muted", git);
          const maximumGitWidth = Math.max(0, Math.floor(width * 0.42));
          const fittedGit = git
            ? truncateToWidth(
                styledGit,
                maximumGitWidth,
            ...
            : "";
          const usageWidth = fittedGit
            ? Math.max(1, width - visibleWidth(fittedGit) - 1)
            : width;
          const usage = renderContextWindowBar(modelInfo, usageWidth, theme);

          const lines = [
            columns(directory, theme.fg("muted", model), width),
            columns(usage, fittedGit, width),
          ];
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    modelInfo = createEmptyModelInfoState();
    gitInfo = createEmptyGitInfoState();
    install(ctx);
  });
